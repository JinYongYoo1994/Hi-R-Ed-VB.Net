...
<%
	...
	Dim mbjFSO,mInStream,mRows,maArrRows  
	Dim mFileName
	Dim mReturnValue
	Dim contact
	...
	Else 
		...
		Do Until mInStream.AtEndOfStream  
			sRows = mInStream.readLine  
			mArrRows = Split(sRows,",")
			dUser = mArrRows(14)
			dPwd = mArrRows(15)
			tUser = """" + Session("USER") + """"
			tPwd = """" + Session("PWD") + """"
			If (dUser = tUser) And (dPwd = tPwd) Then
				mReturnValue = mArrRows
			End If
		Loop
...
line-height:150%">&nbsp;&nbsp;<span style="letter-spacing:1.0pt">“</span><b>Achieving 
	...
	this day
	<!--webbot bot="Timestamp" S-Type="REGENERATED" S-Format="%B %d, %Y" startspan -->September 27, 2019<!--webbot bot="Timestamp" i-checksum="38018" endspan --></span></p>
	<font SIZE="3">
